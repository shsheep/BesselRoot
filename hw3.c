...
#define N 100000 // intervals in [1.0, 10.0]
#define NBMAX 20 // the maximum quantity of roots to find
#define X1 1.0 //   range from
#define X2 10.0 //  range to
...
{
	return pow(EULER, x*PI) - PI;
}

static void eq_4d(float x, float *fn, float *df)
{
	*fn = pow(EULER, x*PI) - PI;
	*df = PI*pow(EULER, x*PI);
}
...
int main(void) {
	...
	// Root finding for Bessel function using various method
	...
	// Root finding for Bessel function using Muller method
	printf("\nMuller method\n%21s %15s\n","x","f(x)");
	for (i=1;i<=nb;i++) {
		xacc=(1.0e-6)*(xb1[i]+xb2[i])/2.0;
		root=rtmuller(fx,xb1[i],xb2[i],xacc);
		printf("root %3d %14.6f %14.6f\n",i,root,fx(root));
	}
	...
	zbrak(eq_4,-2.0,2.0,N,xb1,xb2,&nb);
	printf("\nNR with bracketing method pow(EULER, x*PI) - PI\n%21s %15s\n","x","f(x)");
	for (i=1;i<=nb;i++) {
		xacc=(1.0e-6)*(xb1[i]+xb2[i])/2.0;
		root=rtsafe(eq_4d,xb1[i],xb2[i],xacc);
		printf("root %3d %14.6f %14.6f\n",i,root,fx(root));
	}
